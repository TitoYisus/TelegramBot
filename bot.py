...
def add_dates(message):
    ...
    bot.send_message(
        chat_id=cid,
        text="Envíame un csv (con caption '/s') separado por punto y coma (;) "
...
def actualizar_json(message, recordatorios):
    cid = message.chat.id
    bot.send_message(
        chat_id=cid, text="Parseo finalizado. Actualizando recordatorios..."
    )
    ...
    bot.send_message(
        chat_id=cid,
        text="Todos los recordatorios actualizados. Espero que tengas un día estupendo <3",
    )
    return
